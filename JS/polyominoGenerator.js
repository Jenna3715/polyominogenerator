...
this.returnPolyominos = function (num) {
	return generatePolyominos(Number(num));
}
...
polyominoGeneratorFixed = ((function() {
/**----------------------------------------------------------
				MODEL ELEMENT CONSTRUCTORS
------------------------------------------------------------*/
/** Object factory that creates a point and returns it.
...
var generatePolyominos = function(num) {
	...
	var newPolyominos = [],
	prevPolyominos = generatePolyominos(num - 1); //recursive call here!!
	for(var prevpoly of prevPolyominos) {
		((function(prevpoly){
			var R0 = copyPolyomino(prevpoly),
					R1 = rotatePointsClockWise(copyPolyomino(R0)),
					R2 = rotatePointsClockWise(copyPolyomino(R1)),
					R3 = rotatePointsClockWise(copyPolyomino(R2));
			return [
				R0,
				R1,
				R2,
				R3,
				flipPointsVertically(copyPolyomino(R0)),
				flipPointsVertically(copyPolyomino(R1)),
				flipPointsVertically(copyPolyomino(R2)),
				flipPointsVertically(copyPolyomino(R3)),
				//flipPointsHorizontally(copyPolyomino(R0)),
				//flipPointsHorizontally(copyPolyomino(R1)),
				//flipPointsHorizontally(copyPolyomino(R2)),
				//flipPointsHorizontally(copyPolyomino(R3)),
			];
		})(prevpoly)).forEach(function(testPoly){console.log(newPolyominos)
			newPolyominos=checkAndAddNewPiece(normalizePolyomino(testPoly), newPolyominos);
		});
	}console.log(newPolyominos)
	//var end_time = Date.now();
	//console.log(num, '-omino took: ', end_time - start_time, 'milliseconds');
	return newPolyominos;
}
...
//create a new copy of the polyomino
var copyPolyomino = function(polyomino) {if(polyomino==null)debugger
	return JSON.parse(JSON.stringify(polyomino));
}
//create a new copy of the point array
var copyPointArray = function(pointArray) {
	var newArray = [];
